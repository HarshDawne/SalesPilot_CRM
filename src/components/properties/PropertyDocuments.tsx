...
export function PropertyDocuments({ 
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Use FileReader to encode as Base64 so the URL persists in the DB
        const reader = new FileReader();
        reader.onloadend = () => {
            onAdd({
                name: file.name.replace(/\.[^/.]+$/, ''), // strip extension from display name
                type,
                docType: 'Other',
                url: reader.result as string // Base64 data URL — survives page reload
            });
