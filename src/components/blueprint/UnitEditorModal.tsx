...
import { X, Save, FileText, Trash2, Plus, LayoutGrid, Grid3X3, Zap } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-xs font-black text-slate-500 uppercase tracking-tighter">Carpet Area (sqft)</label>
                            <SmartInput
                                value={form.carpetArea}
                                disabled={isReadOnly}
                                onChange={(val) => {
                                    const builtUp = Math.round(val * 1.15);
                                    setForm({ 
                                        ...form, 
                                        carpetArea: val, 
                                        builtUpArea: builtUp,
                                        superBuiltUpArea: Math.round(builtUp * 1.35)
                                    });
                                }}
                                className="w-full p-2.5 rounded-xl border border-slate-200 bg-white font-bold text-slate-900 outline-none"
                            />
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-black text-slate-500 uppercase tracking-tighter">Facing</label>
                            <select
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Enterprise Pricing Engine</span>
                        </div>
                        
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-400 uppercase">Base Price / sqft (₹)</label>
                                <SmartInput
                                    value={form.basePrice || 0}
                                    disabled={isReadOnly}
                                    onChange={(val) => setForm({ ...form, basePrice: val })}
                                    className="w-full p-2 bg-white rounded-lg border border-slate-200 font-bold text-slate-700 outline-none"
                                />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-400 uppercase">Floor Rise (₹)</label>
                                <SmartInput
                                    value={form.floorRise || 0}
                                    disabled={isReadOnly}
                                    onChange={(val) => setForm({ ...form, floorRise: val })}
                                    className="w-full p-2 bg-white rounded-lg border border-slate-200 font-bold text-slate-700 outline-none"
                                />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-400 uppercase">PLC Charges (₹)</label>
                                <SmartInput
                                    value={form.plcCharges || 0}
                                    disabled={isReadOnly}
                                    onChange={(val) => setForm({ ...form, plcCharges: val })}
                                    className="w-full p-2 bg-white rounded-lg border border-slate-200 font-bold text-slate-700 outline-none"
                                />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-400 uppercase">Net Total (Calculated)</label>
                                <div className="p-2 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-100 font-black">
                                    ₹{(( (form.basePrice + form.floorRise) * (form.builtUpArea || 1200) ) + (form.plcCharges || 0)).toLocaleString()}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <ThreeDRenderSection
                        ...
                        onDeleteRequest={async (requestId) => {
                            try {
                                ...
                                if (data.success) {
                                    showToast("Render request deleted", "success");
