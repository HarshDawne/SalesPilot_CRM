...
export default function SeedPage() {
    ...
    const runSeed = async () => {
        ...
        for (const prop of properties) {
            try {
                log(`Creating ${prop.name}...`);

                // 1. Create Property
                const propConfig = { ...prop };
                delete (propConfig as any).towers;

                const propRes = await propertiesAPI.create(propConfig as any);
                const propId = propRes.id || propRes.data?.id;

                if (propId) {
                    log(`✅ Created Property ID: ${propId}`);

                    // 2. Create Towers
                    for (const tower of prop.towers) {
                        const towerConfig = { ...tower };
                        delete (towerConfig as any).units;

                        const towerRes = await propertiesAPI.createTower(propId, towerConfig);
                        const towerId = towerRes.data?.id;

                        if (towerId) {
                            log(`  ✅ Created Tower: ${tower.name}`);

                            // 3. Create Units
                            for (const unit of tower.units) {
                                await propertiesAPI.createUnit(towerId, { ...unit, propertyId: propId });
                            }
                            log(`    ✅ Created ${tower.units.length} units`);
                        }
                    }
                }
            } catch (err) {
                log(`❌ Failed to seed ${prop.name}: ${err}`);
            }
        }
        setStatus('Complete');
    };
    ...
    return (
        <div className="p-8">
            <h1 className="text-2xl font-bold mb-4">Seed Properties</h1>
            <button
                onClick={runSeed}
                disabled={status === 'Seeding...'}
                className="px-4 py-2 bg-blue-600 text-white rounded disabled:opacity-50"
            >
                {status === 'Idle' ? 'Run Seed' : status}
            </button>

            <div className="mt-4 p-4 bg-slate-100 rounded h-96 overflow-auto font-mono text-sm">
