...
// Global queue to ensure atomic read-modify-write cycles
let dbQueue = Promise.resolve();

async function enqueue<T>(op: () => T | Promise<T>): Promise<T> {
    const result = dbQueue.then(op);
    dbQueue = result.then(() => {}).catch(() => {});
    return result;
}
...
export const db = {
    users: {
        ...
        update: (id: string, updates: Partial<User>) => enqueue(() => {
            const data = readDb();
    ...
    leads: {
        ...
        update: (id: string, updates: Partial<Lead>) => enqueue(() => {
            const data = readDb();
    ...
    timeline: {
        ...
        create: (event: TimelineEvent) => enqueue(() => {
            const data = readDb();
    ...
    activities: {
        ...
        create: (activity: Activity) => enqueue(() => {
            const data = readDb();
    ...
    bookings: {
        ...
        create: (booking: Booking) => enqueue(() => {
            const data = readDb();
            data.bookings.push(booking);
            writeDb(data);
            return booking;
        }),
        update: (id: string, updates: Partial<Booking>) => enqueue(() => {
            const data = readDb();
    ...
    properties: {
        ...
        create: (property: Property) => enqueue(() => {
            const data = readDb();
    ...
    projects: {
        ...
        create: (project: Project) => enqueue(() => {
            const data = readDb();
    ...
    callLogs: {
        ...
        create: (callLog: CallLog) => enqueue(() => {
            const data = readDb();
    ...
    campaigns: {
        ...
        create: (campaign: Campaign) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<Campaign>) => enqueue(() => {
            const data = readDb();
            if (!data.campaigns) data.campaigns = [];
    ...
    campaignJobs: {
        ...
        create: (job: CampaignJob) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<CampaignJob>) => enqueue(() => {
            const data = readDb();
            if (!data.campaignJobs) data.campaignJobs = [];
    ...
    providerConfigs: {
        ...
        create: (config: ProviderConfig) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<ProviderConfig>) => enqueue(() => {
            const data = readDb();
            if (!data.providerConfigs) data.providerConfigs = [];
    ...
    whatsappMessages: {
        ...
        create: (msg: WhatsAppMessage) => enqueue(() => {
            const data = readDb();
    ...
    agentAvailability: {
        ...
        create: (availability: AgentAvailability) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<AgentAvailability>) => enqueue(() => {
            const data = readDb();
            if (!data.agentAvailability) data.agentAvailability = [];
    ...
    projectAvailability: {
        ...
        create: (availability: ProjectAvailability) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<ProjectAvailability>) => enqueue(() => {
            const data = readDb();
            if (!data.projectAvailability) data.projectAvailability = [];
    ...
    visitParticipants: {
        ...
        create: (participant: VisitParticipant) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<VisitParticipant>) => enqueue(() => {
            const data = readDb();
            if (!data.visitParticipants) data.visitParticipants = [];
    ...
    notificationPreferences: {
        ...
        create: (pref: NotificationPreference) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<NotificationPreference>) => enqueue(() => {
            const data = readDb();
            if (!data.notificationPreferences) data.notificationPreferences = [];
    ...
    blackoutDates: {
        ...
        create: (blackout: BlackoutDate) => enqueue(() => {
            const data = readDb();
    ...
    visitConflicts: {
        ...
        create: (conflict: VisitConflict) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<VisitConflict>) => enqueue(() => {
            const data = readDb();
            if (!data.visitConflicts) data.visitConflicts = [];
    ...
    visitReminderJobs: {
        ...
        create: (job: VisitReminderJob) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<VisitReminderJob>) => enqueue(() => {
            const data = readDb();
            if (!data.visitReminderJobs) data.visitReminderJobs = [];
    ...
    propertyManagement: {
        ...
        create: (property: PropertyManagement) => enqueue(() => {
            const data = readDb();
            if (!Array.isArray(data.propertyManagement)) {
        ...
        update: (id: string, updates: Partial<PropertyManagement>) => enqueue(() => {
            const data = readDb();
    ...
    towers: {
        ...
        update: (id: string, updates: Partial<Tower>) => enqueue(() => {
            const data = readDb();
            if (!data.towers) data.towers = [];
    ...
    units: {
        ...
        update: (id: string, updates: Partial<Unit>) => enqueue(() => {
            const data = readDb();
            if (!data.units) data.units = [];
    ...
    unitReservations: {
        ...
        create: (reservation: UnitReservation) => enqueue(() => {
            const data = readDb();
        ...
        update: (id: string, updates: Partial<UnitReservation>) => enqueue(() => {
            const data = readDb();
            if (!data.unitReservations) data.unitReservations = [];
    ...
    renderRequests: {
        findAll: () => readDb().renderRequests || [],
        findById: (id: string) => (readDb().renderRequests || []).find(r => r.id === id),
        create: (request: RenderRequest) => enqueue(() => {
            const data = readDb();
            if (!Array.isArray(data.renderRequests)) data.renderRequests = [];
            data.renderRequests.push(request);
            writeDb(data);
            return request;
        }),
        update: (id: string, updates: Partial<RenderRequest>) => enqueue(() => {
            const data = readDb();
            if (!Array.isArray(data.renderRequests)) data.renderRequests = [];
...
// Export leadService for backward compatibility/API usage
export const leadService = {
    getAllResults: () => db.leads.findAll(),
    getById: (id: string) => db.leads.findById(id),
    update: (id: string, updates: any) => db.leads.update(id, updates),
    create: (lead: any) => db.leads.create(lead)
};
